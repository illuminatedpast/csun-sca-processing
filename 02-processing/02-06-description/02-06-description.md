...
Some elements should be used in all finding aids, while others are optional, and should be used only where necessary to enhance patrons' ability to understand and use the collection. Mandatory elements are marked *Required*. The notes fields should have all "publish" checkboxes checked by default in the ASpace Resource record template. Note that the collection-level publish checkbox under "Basic Information" should remain unchecked until the finding aid is reviewed and sent to cataloging. Finding aids may include the following elements (additional [DACS fields](https://saa-ts-dacs.github.io/) may be used after consultation with the SC/A Librarian or Head):
